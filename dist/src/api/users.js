...
// @access  public
router.post("/", [
    express_validator_1.check("name").not().isEmpty().withMessage("please enter a name"),
    express_validator_1.check("email").isEmail().withMessage("please provide a valid email"),
    ...
    return __generator(this, function (_c) {
        switch (_c.label) {
            ...
            case 5:
                _c.sent(); // save user object to the database
                payload = {
                    user: {
                        id: user.id,
                    },
                };
                jsonwebtoken_1.default.sign(payload, config_1.default.get("secret"), { expiresIn: 3600 }, function (err, token) {
                    if (err) {
                        throw err;
                    }
                    else {
